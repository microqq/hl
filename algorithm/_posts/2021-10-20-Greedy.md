...
- [3. 经典例题](#3-经典例题)
  - [3.1 霍夫曼编码](#31-霍夫曼编码)
    ...
    - [3.1.3 代码实现](#313-代码实现)
  - [3.2 跳跃游戏](#32-跳跃游戏)
    - [3.2.1 问题描述(lintCode-116)：](#321-问题描述lintcode-116)
    - [3.2.2 分析](#322-分析)
    - [3.2.3 代码实现](#323-代码实现)
    - [3.2.4 跳跃游戏 II](#324-跳跃游戏-ii)
- [3.3 剪绳子](#33-剪绳子)
- [参考](#参考)
...
#### 3.2.1 [问题描述(lintCode-116)](https://www.lintcode.com/problem/116/)：
...
之前动态规划的那篇文章也求解了跳跃问题，从后向前建立状态转移方程，时间复杂度是 $O(n^2)$。贪心法怎么求解该问题呢？

直观的感觉是每一步我们都应该争取跳到最远的位置上去，假设当前位置是 $i$，那么最远可跳达的位置是 $A(i) + i$。首先，如果当前位置即是目标位置，那么问题直接得解，否则，如果当前位置不是目标位置，那么当前位置的最大可跳跃距离一定不能为 $0$。前面假设了当前位置是 $i$，最远可跳达的位置是 $A(i) + i$，也就是说范围 $Range = [i+1, A(i) + i]$ 内都是当前可跳达的位置，**因此，问题变成了我们需要站在当前位置 $i$ 去选择应该跳到范围 $Range = [i+1, A(i) + i]$ 内的某个位置上去**，怎么选择呢？每一步我们都要争取能够跳到最远的位置上去，所以，我们可以选择当 $jump = A[j] +j, (i+1 \leq j \leq A(i) +1)$ 最大时的位置。
...
bool canJump(vector<int> &A) {
    ...
    int i = 0;
    while (i < length) {
        int jumpDistance = A[i] + i;
        if (jumpDistance == i) {
            return false;
        }
        int k = std::min(jumpDistance, length - 1);
        int maxIndex = i;
        jumpDistance = std::numeric_limits<int>::min();
        for (int j = i + 1; j <= k; j++) {
            if (jumpDistance < A[j] + j) {
                jumpDistance = A[j] + j;
                maxIndex = j;
            }
        }
...
跳跃游戏问题的一个变体是统计最少的跳跃步数，上面使用贪心法的解法已经可以统计最少跳跃步数了，代码只需要小小改动一下即可：
...
- [问题描述:](https://www.nowcoder.com/practice/106f666170554379ab1974e5a601e741?tpId=13&tqId=2254224&ru=/ta/coding-interviews&qru=/ta/coding-interviews/question-ranking)

给你一根长度为 n 的绳子，请把绳子剪成整数长的 m 段（ m 、 n 都是整数， n > 1 并且 m > 1 ， m <= n ），每段绳子的长度记为 k[1],...,k[m] 。请问 k[1]*k[2]*...*k[m] 可能的最大乘积是多少？例如，当绳子的长度是 8 时，我们把它剪成长度分别为 2、3、3 的三段，此时得到的最大乘积是 18 。

由于答案过大，请对 998244353 取模。

数据范围：2≤n≤10142\leq n \leq 10^{14}2≤n≤1014
进阶：空间复杂度 O(1)O(1)O(1) ， 时间复杂度 O(logn)O(logn)O(logn)
...
```c++
// 据观察，当长度是2、或者3时，不应在切割绳子，可直接得到结果
// 贪心法，局部最优解可获得整体最优解，f(3) 相对 f(2) 更大，
// 每一次从 number 中截取长度为 3 的绳子
// 任何数字都可以拆成2的倍数加上3的倍数
// x = n % 3 时，若x等于1时，不允许这样拆分
long long cutRope(long long number) {
    ...
    int mod = number % 3;
    long long timesOf3 = number / 3;
    if (mod == 1) {
        timesOf3 -= 1;
    }
    
    long long timesOf2 = (number - timesOf3 * 3) / 2;
    long long result = 1;
    if (timesOf3 != 0) {
        result = getPow(3, timesOf3);
    }
    if (timesOf2 != 0) {
        result = (result * getPow(2, timesOf2)) % (long long)998244353;
    }
...
long long getPow(long long x, long long y) {
    ...
    long long result = getPow(x, y / 2) % 998244353;
    result = (result * result) % 998244353;
    result = (result * getPow(x, y % 2)) % 998244353;
    y = y / 2;
